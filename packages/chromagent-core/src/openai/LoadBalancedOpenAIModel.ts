...
/**
 * This class provide randomized load balancing capability for OpenAI Model.
 * It takes in an OpenAIModelRegistry and a list of model name + weight.
 ...
 * The check should use isValidResponse()
 * This process continue until the last model, then their response / error is simply returned
 * back to caller.
 *
 */
export class LoadBalancedOpenAIModel implements OpenAIModel {
    private readonly registry: OpenAIModelRegistry;
    private models: ModelWithWeight[];
    private readonly errorTimeoutMs: number;
    private failedModels: Map<string, number> = new Map();

    constructor(registry: OpenAIModelRegistry, models: ModelWithWeight[], errorTimeoutMs: number = 60000) {
    ...
    async chatCompletion(request: OpenAIChatCompletionsRequest): Promise<OpenAIChatCompletionsResponse | AsyncIterable<OpenAIChatCompletionChunk>> {
        ...
        while (modelsToTry.length > 0) {
            ...
            try {
                const response = await model.chatCompletion(request);
    ...
    private isValidResponse(response: OpenAIChatCompletionsResponse | AsyncIterable<OpenAIChatCompletionChunk>): boolean {
        if (Symbol.asyncIterator in response) {
            // For streaming response, we assume it's valid if the promise resolved
            return true;
        }
        const r = response as OpenAIChatCompletionsResponse;
        return !!(r && r.choices && r.choices.length > 0);
    }
}
